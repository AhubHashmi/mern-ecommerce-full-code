        ...
        <>
            <nav className="navbar navbar-expand-lg navbar-light bg-body-tertiary">
                <div className="container-fluid">
                    <Link to="/" className="navbar-brand"> <AiOutlineShoppingCart /> SHOP ON!</Link>
                        ...
                        <ul className="navbar-nav ms-auto mb-2 mb-lg-0">
                            ...
                            <li className="nav-item dropdown">
                                ...
                                <ul className="dropdown-menu">
                                    ...
                                    {categories?.map((c) => (
                                        <li key={c.id}><Link className="dropdown-item" to={`/category/${c.slug}`}>{c.name}</Link></li>
                                    ))}
                                </ul>
                            </li>
                            {!auth.user ? (
                                ...
                                <>
                                    <li className="nav-item dropdown">
                                        <NavLink className="nav-link dropdown-toggle" to="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">{auth?.user?.name}</NavLink>
                                        <ul className="dropdown-menu">
                            ...
                            <li className="nav-item">
                                <NavLink to="/cart" className="nav-link">Cart <Badge count={cart?.length} showZero offset={[10, -5]} /></NavLink>
                            </li>
                        </ul>
                        <SearchInput />
                    </div>
